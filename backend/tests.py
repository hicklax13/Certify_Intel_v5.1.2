...
try:
    ...
    test_pass("Crunchbase Scraper", f"Funding: {data['crunchbase']['total_funding']}")
    test_pass("News Scraper", f"Articles: {len(data['news'])}")
    test_pass("Job Scraper", f"Postings: {len(data['jobs']['postings'])}")
except Exception as e:
...
try:
    ...
    test_pass("Battlecard PDF Generation", f"File exists: {exists}, Path: {path}")
except Exception as e:
...
try:
    ...
    r = httpx.get(f"{base_url}/api/dashboard/stats", timeout=5)
    test_pass("Dashboard Stats", f"Status: {r.status_code}")
    
    # Test competitors list
    r = httpx.get(f"{base_url}/api/competitors", timeout=5)
    count = len(r.json()) if r.status_code == 200 else 0
    test_pass("Competitors List", f"Status: {r.status_code}, Count: {count}")
    
    # Test export JSON
    r = httpx.get(f"{base_url}/api/export/json", timeout=5)
    test_pass("JSON Export", f"Status: {r.status_code}")
    
except httpx.ConnectError:
    test_fail("API Endpoints", "Server not running on localhost:8000")
except Exception as e:
