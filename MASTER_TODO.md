...
| - | Fix .env loading from exe directory | 2026-01-25 | Modified __main__.py |
...
### For Claude Agents
1. **Before starting work**: Check "Active Tasks" for current priorities
2. **When starting a task**: Move task to "Active Tasks" with your session ID
3. **When completing a task**: Move to "Completed Tasks" with date
4. **When blocked**: Add notes to the task and mark as `blocked`
...
*Updated By: Claude Agent (session: review-launch-options-uvBKJ)*
